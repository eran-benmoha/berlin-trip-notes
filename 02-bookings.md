# Bookings & Confirmations — Berlin (18.2 → 22.2)

Quick nav: [Trip overview](00-trip-overview.html) · [Itinerary](01-itinerary.html) · [Home](index.html)

## Lodging

| Item | Provider | Confirmation | Dates | Address | Notes |
|---|---|---|---|---|---|
| Lodging |  |  | 18.2 → 22.2 | Wolliner Str 12b, Berlin | Closest transit: M10 “Wolliner Str.”, U8 “Bernauer Straße” |

## Transport (Flights / Trains)

| Leg | Provider | Booking/PNR | Date | Time | From → To | Notes |
|---|---|---|---|---|---|---|
| Arrival | El Al (LY) |  | 18.2 | Arrive **09:30** | TLV (Ben Gurion) → BER | Add: flight #, terminal, baggage info. |
| Departure | El Al (LY) |  | 22.2 | Depart **~21:30** | BER → TLV | Add: flight #, terminal, baggage drop cutoff. |

### Getting to/from BER (public transport)
- **Arrival (18.2)**: BER → **FEX or RE** to **Berlin Hbf** (~30 min) → **Tram M10** to "Wolliner Str." (~15 min). Total ~45 min.
- **Departure (22.2)**: **Tram M10** from "Wolliner Str." → **Berlin Hbf** (~15 min) → **FEX or RE** to BER (~30 min). Total ~45 min. Leave by **~19:00**.
  - If coming from Charlottenburg: **S41/S42** to "Südkreuz" (~20 min) → **FEX** to BER (~20 min).
- All covered by **WelcomeCard ABC** (zone C includes BER).

### Calendar reminder template (fill once you confirm exact times)
- Flight TLV → BER (LY____) — **Depart** __:__ • **Arrive** 09:30
  - Notes: PNR ____ • Terminal/Gate ____ • Bag drop cutoff ____
- Flight BER → TLV (LY____) — **Depart** ~21:30
  - Notes: PNR ____ • Terminal/Gate ____ • Bag drop cutoff ____ • “Leave hotel” time target ____

## Local transit

| Item | Type | Bought? | Valid | Notes |
|---|---|---:|---|---|
| BVG ticket | Berlin WelcomeCard |  | 18.2 → 22.2 | Consider **WelcomeCard ABC (5 days)** if you want **BER + Potsdam** included (prices seen ~€55.90–€59.50). |

## Attractions / tickets

| Item | Date | Time | Booking link | Confirmation | Notes |
|---|---|---|---|---|---|
| Reichstag dome |  |  |  |  | Bring ID (often required) |
| Museum Island (museum: ___) |  |  |  |  | Time slots may sell out |
| Free walking tour — “Welcome to Berlin!” (English) | 18.2 | 14:30–17:15 |  | Booking **#333455727** | Meeting point: **Rotes Rathaus (Red Town Hall)** — look for the **yellow umbrella**. (Plan A) |
| Free walking tour — “Welcome to Berlin!” (English) | 19.2 | 10:00–12:45 |  | Booking **#333455829** | Same meeting point (**Rotes Rathaus**, yellow umbrella). Backup in case arrival day is too tight. |

## Restaurants (only if you book)

| Place | Date | Time | Booking name | Confirmation | Notes |
|---|---|---|---|---|---|
|  |  |  |  |  |  |

## Day trip (if applicable)

| Trip | Date | Tickets/route | Confirmation | Notes |
|---|---|---|---|---|
| Potsdam |  |  |  |  |
| Sachsenhausen |  |  |  |  |
